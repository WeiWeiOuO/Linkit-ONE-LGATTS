...
boolean LGATTSUart::onCharacteristicAdded(LGATTAttributeData &data)
{
    ...
    APP_LOG("[LGATTS ino]LGATTSUart::onCharacteristicAdded f[%d] uuid[12] = [0x%x] len[%d]", data.failed, uuid->uuid[12], uuid->len);
...
boolean LGATTSUart::onConnection(const LGATTAddress &addr, boolean connected)
{
    ...
    APP_LOG("[LGATTS ino]LGATTSUart::onConnection connected [%d], [%x:%x:%x:%x:%x:%x]", _connected, 
...
{
    APP_LOG("[LGATTS ino]LGATTSUart::onRead _connected [%d]", _connected);
    if (_connected)
    {
        ...
        APP_LOG("[LGATTS ino]LGATTSUart::onRead onRead [%d][%s]", value.len, value.value);
...
{
    APP_LOG("[LGATTS ino]LGATTSUart::onWrite _connected [%d]", _connected);
    // todo read UART data.
    if (_connected)
    {
